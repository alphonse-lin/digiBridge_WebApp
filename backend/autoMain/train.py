...
# 设置日志级别为 ERROR，这样就不会显示 WARNING 级别的消息
logging.getLogger('tensorflow').setLevel(logging.ERROR)

seed = 111
np.random.seed(seed)
tf.random.set_seed(seed)
weight_path=os.path.join(os.path.dirname(__file__), 'ic_model')
...
def init_model():
    # Load the dataset
    captions_mapping, text_data = load_captions_data(token_path)
    ...
    return caption_model,index_lookup,max_decoded_sentence_length,valid_images,vectorization
...
def generate_caption(image):
    caption_model,index_lookup,max_decoded_sentence_length,valid_images,vectorization=init_model()
...
if __name__ == '__main__':
    weight_path="backend/autoMain/ic_model"
    token_path="backend/autoMain/token.txt"
    res=generate_caption(r"backend/autoMain/image/image_2.jpg")
    print(res)
