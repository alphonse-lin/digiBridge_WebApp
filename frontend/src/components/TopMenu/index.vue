    ...
    <el-menu mode="horizontal"
      ...
      <div v-for="item in menus"
        ...
        <el-sub-menu v-if="item.children && item.children.length > 0"
          ...
          <el-menu-item v-for="v in item.children"
                        :key="v.id"
                        :index="v.id">{{ v.label }}</el-menu-item>
        </el-sub-menu>
        <el-menu-item v-else
                      :index="item.id">{{ item.label }}</el-menu-item>
      </div>
    </el-menu>

    <IframeWrap_AutoMSG ref="refIframeWrap_AutoMSG"
                :selected="selected"></IframeWrap_AutoMSG>
    <IframeWrap_KBMain ref="refIframeWrap_KBMain"
                :selected="selected"></IframeWrap_KBMain>
  </div>
</template>
